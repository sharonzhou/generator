...
class BEGANGenerator(nn.Module):
    def __init__(self, opt=None, disc=False):
        super(BEGANGenerator, self).__init__()
        self.num_channel = 64 #opt.nc
        self.b_size = 16 #opt.b_size
        self.h = 64 #opt.h
        self.disc = disc
        self.t_act = 1 #opt.tanh
        self.scale_size = 64 #opt.scale_size
...
class Discriminator(nn.Module):
    def __init__(self, nc):
        ...
        self.enc = Encoder(nc)
        self.dec = BEGANGenerator(nc, True)
    def forward(self, input):
